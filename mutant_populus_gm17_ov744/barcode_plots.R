...
args[1] <- "oe22_gm17_ov744"
args[2] <- "0"
args[3] <- "sample_metadata_oe22_gm17_ov744.csv"
...
cluster_reads_long$Structure <- meta_samples$Structure
cluster_reads_long$Type <- meta_samples$Type
...
cluster_reads_long_barcoded <- cluster_reads_long %>% select(Sample,Barcode,Unknown,Experiment,Structure,Type,Replicate) %>% gather(key="BarcodePresence", value="Reads",2:3)
...
p <- ggplot(cluster_reads_long_barcoded, aes(x=Replicate, y=Reads, fill=BarcodePresence)) +
  scale_fill_manual(values = c("Grey","violetred4")) +
  facet_grid(Type~Structure, drop=T, scales="free_x", space="free") +
  geom_bar(position="stack", stat="identity") +
  xlab("Sample") + 
...
plotting_df <- clusters_pct_threshold_meta_long %>% group_by(Library,Sample,Structure,Type) %>% summarize(Percentage=sum(Percentage))
...
p <- ggplot(clusters_pct_threshold_meta_long %>% group_by(Library,Sample,Structure,Type) %>% 
...
p <- ggplot(clusters_pct_threshold_meta_long %>% group_by(Library,Replicate,Structure,Type) %>% 
...
#PLOT - Bar charts of percentage of all barcodes identified in the meta_barcodes file coordinated in grid Type vs Structure
p <- ggplot(clusters_pct_threshold_meta_long %>% filter(.$Library %in% list_known_libraries)  %>% group_by(Library,Replicate,Type,Structure) %>% 
              summarize(Percentage=sum(Percentage)), aes(x=Replicate, y=Percentage, fill=Library)) +
  scale_fill_manual(values = library_colors) +
  geom_bar(position="fill", stat="identity") +
  facet_grid(Type~Structure, drop=T, scales="free_x", space="free") +
...
histo_df <- cluster.csv %>% select(Center,all_of(sample_column_first):all_of(sample_column_last)) %>% 
                            ...
                            separate(.,Sample,c(NA,NA,"Sample")) %>% 
                            transform(., Sample = as.integer(Sample)) %>%
                            left_join(.,meta_samples) %>% select(Center,Library,Structure,Type,Replicate,Reads)

#where should y-axis be cut off?
histo_cutoff <- 500

#inoculum
p <- ggplot(histo_df %>% filter(Structure=="Inoculum"), aes(x=Reads)) + 
...
for (structure_ in levels(clusters_pct_threshold_meta_long$Structure)[-1]) { 
  
  for (type_ in levels(clusters_pct_threshold_meta_long$Type)[-4]) {
    
      for (replicate_ in levels(as.factor(clusters_pct_threshold_meta_long$Replicate))) {
    
      histograms <- ggplot(histo_df %>% filter(Structure==structure_, Type==type_, Replicate==replicate_), aes(x=Reads)) + 
                geom_histogram(color="red", fill="white", binwidth=1) +
                xlab("Reads per Barcode") + 
                ylab("Number of Barcodes") +
                coord_cartesian(ylim=c(0, histo_cutoff)) +
                ...
                theme(axis.text.x = element_text(angle = 0, hjust = .5)) +
                theme(legend.position="bottom")  
      ggsave(filename = file.path("results/distribution",paste(paste(args[1],"histo",structure_,type_,replicate_,sep="_"),".pdf",sep="")), plot = histograms, device = "pdf" ,width = 297, height = 210, units = "mm",useDingbats = FALSE)
  
      }
  }
}
rm(temp,list_structures,list_types)


p <- ggplot(histo_df %>% filter(Structure=="Primary", Type=="Tip", Replicate=="A"), aes(x=Reads)) + 
  geom_histogram(color="red", fill="white", binwidth=1) +
  xlab("Reads per Barcode") + 
  ylab("Number of Barcodes") +
  coord_cartesian(ylim=c(0, histo_cutoff)) +
...
p <- ggplot(histo_df %>% filter(Structure=="Primary", Type=="Tip", Replicate=="B"), aes(x=Reads)) + 
  geom_histogram(color="red", fill="white", binwidth=1) +
  xlab("Reads per Barcode") + 
  ylab("Number of Barcodes") +
  coord_cartesian(ylim=c(0, histo_cutoff)) +
...
p <- ggplot(histo_df %>% filter(Structure=="Primary", Type=="Tip", Replicate=="C"), aes(x=Reads)) + 
  geom_histogram(color="red", fill="white", binwidth=1) +
  xlab("Reads per Barcode") + 
  ylab("Number of Barcodes") +
  coord_cartesian(ylim=c(0, histo_cutoff)) +
